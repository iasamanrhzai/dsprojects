...
class DataFrameHandler:
    ...
    @staticmethod
    def encode_label_columns(df, columns, encodedcolumns):
        """
        Encodes specified columns in the DataFrame using a LabelEncoder.
        ...
        pd.DataFrame: The DataFrame with encoded columns.
        """
        if len(columns) != len(encodedcolumns):
            raise ValueError("The number of columns and encoded column names must be the same.")
        
        for col, new_col in zip(columns, encodedcolumns):
            le = LabelEncoder()
